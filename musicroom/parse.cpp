...
bool GameInfo::ParseTrackDataEx(ConfigFile& NewGame)
{
	...
	if(Save)
	{
		...
		Str.format("%s의 트랙 정보를 새로 저장하는 중입니다...", FXPath::name(NewGame.GetFN()));
		BGMLib::UI_Stat(Str);
		if(!NewGame.Save())
		{
			Str.format("\n%s 파일!\n", BGMLib::WriteError);
...
// -----------------
FXString FNPattern;
const FXString Token[] = {"number", "name_jp", "name_en", "name_ko", "name"};
...
FXString PatternFN(TrackInfo* Track)
{
	...
	FN.substitute(GetToken(3), Track->Name[LANG_KO]);
	FN.substitute(GetToken(4), Track->Name[Lang]);
