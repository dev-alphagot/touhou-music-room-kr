...
bool SeekTest::Scan(GameInfo* GI)
{
	...
	if(Seek == 0)
	{
		Str.append(L"BGM 파일 " + GI->BGMFile + "(이)가 잘못되었습니다.\n");
		Ret = false;
	}
	else if(Seek != GI->Track.Size())
	{
		Str.append(L"이 게임은 체험판입니다. 추출은 체험판에서 들을 수 있는 노래로 제한됩니다.\n");
	}
	BGMLib::UI_Stat(Str);
...
bool SilenceScan::Scan(GameInfo* GI)
{
	...
	BGMLib::UI_Stat(L"트랙 시작 오프셋 값 탐색 중...");
